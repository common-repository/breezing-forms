...
if(
	...
	if ($runmode==_FF_RUNMODE_FRONTEND) {
		...
		if (isset($ff_applic) && $ff_applic=='mod_facileforms') {
			...
			$editable = isset($plg_editable) ? intval($plg_editable) : 0;
			$editable_override = isset($plg_editable_override) ? intval($plg_editable_override) : 0;
			$left     = '';
			$top      = '';
		...
		if (false) {
			...
			if (@file_exists($sourcePath) && @is_readable($sourcePath) && @is_dir($sourcePath) && $handle = @opendir($sourcePath)) {
				...
				@closedir($handle);
                        ...
                        // purge payment cache
                        $sourcePath = WP_CONTENT_DIR . '/breezingforms/payment_cache/';
                        if (@file_exists($sourcePath) && @is_readable($sourcePath) && @is_dir($sourcePath) && $handle = @opendir($sourcePath)) {
                            while (false !== ($file = @readdir($handle))) {
                                if($file!="." && $file!="..") {
                                    $parts = explode('_', $file);
...
} else if(JRequest::getBool('confirmPayPalIpn')){
...
} else if(JRequest::getBool('confirmPayPal') ){
			...
			if($element['internalType'] == 'bfPayPal'){
				...
				if (strcmp ($lines[0], "SUCCESS") == 0) {
					...
					if ($checkPP && ( ( $options['amount'] > 0 && $keyarray['mc_gross'] != (doubleval($options['amount'])+doubleval($options['tax'])) ) || $keyarray['mc_currency'] != strtoupper($options['currencyCode']) ) ) {
						...
						require_once(WP_CONTENT_DIR . '/breezingforms/downloadtpl/error.php');
						...
						if (count($txid) != 0) {
							...
							if($txid[0]->paypal_tx_id == ''){
								...
								if($options['downloadableFile']){
									...
									require_once(WP_CONTENT_DIR . '/breezingforms/downloadtpl/download.php');
...
} else if(JRequest::getBool('paypalDownload') ){
				...
				foreach($area['elements'] As $element){
					if($element['internalType'] == 'bfSofortueberweisung'){
						...
						if($options['downloadableFile']){
							...
							require_once(WP_CONTENT_DIR . '/breezingforms/downloadtpl/sofort_download.php');
						}
						else {
							if($options['thankYouPage'] != ''){
								BFRedirect($options['thankYouPage']);
...
}  else if(JRequest::getBool('sofortueberweisungDownload') ){
        ...
        if($formIdCount > 0){
            ...
            if( @file_exists( $targetPath ) && @is_dir( $targetPath ) ){
                ...
                if(@move_uploaded_file($tempFile,$targetFile)){
                    ...
                    $dataObject = json_decode(base64_decode($objectList[0]->template_code), true);
