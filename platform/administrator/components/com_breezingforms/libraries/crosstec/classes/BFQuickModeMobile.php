...
class BFQuickModeMobile{
	...
	public function process(&$dataObject, $parent = null, $parentPage = null, $index = 0, $childrenLength = 0){
        ...
        public function headers(){
                ...
                $this->addScript(BF_PLUGINS_URL . '/'.BF_FOLDER.'/platform/components/com_breezingforms/libraries/jquery/jq.min.js');
                
                $this->addStyleSheet( WP_CONTENT_URL . '/breezingforms/themes/jq.mobile.min.css' );
                $this->addScript(BF_PLUGINS_URL . '/'.BF_FOLDER.'/platform/components/com_breezingforms/libraries/jquery/jq.mobile.min.js');
	...
	public function render(){
                ...
                $regularReturn = base64_decode(JRequest::getVar('return',''));
                $regularReturn = (strstr($regularReturn,'?non_mobile=1') !== false ? str_replace('?non_mobile=1','',$regularReturn) : str_replace('&non_mobile=1','',$regularReturn));
                $regularReturn = $regularReturn.(strstr($regularReturn,'?') !== false ? '&' : '?') . 'non_mobile=1';
                echo '<a rel="external" href="'.($this->forceMobileUrl != '' ? $return_url . '&forceReturn=' . base64_encode($this->forceMobileUrl) : $regularReturn).'" data-role="button" data-icon="back">'.BFText::_('COM_BREEZINGFORMS_DESKTOP').'</a>';
                echo '</div>';
